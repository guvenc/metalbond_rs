...
async fn test_remove_route() {
    ...
    let remaining = route_table
        .remove_next_hop(vni, dest, next_hop, peer)
        .await
        .unwrap();
...
async fn test_multiple_next_hops() {
    ...
    // Remove one next hop
    let remaining = route_table
        .remove_next_hop(vni, dest, next_hop1, peer)
        .await
        .unwrap();
    ...
    let exists1 = route_table
        .next_hop_exists(vni, dest, next_hop1, peer)
        .await;
    assert!(!exists1, "next_hop1 should be removed");

    // Check if next_hop2 still exists
    let exists2 = route_table
        .next_hop_exists(vni, dest, next_hop2, peer)
        .await;
    ...
    assert_eq!(
        actual_remaining, 1,
        "There should be exactly 1 next hop remaining"
    );
    assert_eq!(
        next_hops[0], next_hop2,
        "If a next hop remains, it should be next_hop2"
    );
    ...
    println!(
        "Reported remaining: {}, Actual remaining: {}",
...
async fn test_same_route_different_peers() {
    ...
    let peer1 = Some(SocketAddr::new(
        IpAddr::V4(Ipv4Addr::new(192, 168, 1, 10)),
        4711,
    ));
    let peer2 = Some(SocketAddr::new(
    ...
    let exists = route_table
        .next_hop_exists(vni, dest, next_hop, peer2)
        .await;
    assert!(exists);
